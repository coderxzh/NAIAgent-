...
当前版本能力对应路线图 [GEO_Agent_长期开发路线图.md](GEO_Agent_长期开发路线图.md) 的 Phase 2 起逐步交付，MVP 范围聚焦在 Phase 2–Phase 7，先形成「知识库 → 事实 → 文章 → 人工审核」的最小闭环。Phase 8 及以后的能力按后续迭代推进，不在当前 Sprint 一次完成。
...
> **MVP 边界**：Phase 2–7 为当前 MVP 范围。Phase 7（问题池、信源发现与文章生成 MVP）完成后，即可形成「知识库 → 事实 → 文章 → 人工审核」的最小闭环；Phase 8 及以后的能力在本次 Sprint 后冻结，不进入 MVP。

#### Phase 7：问题池、信源发现与文章生成 MVP（支持类文章与综合排行榜）
...
26. Claim 初筛审核
27. GEO 风格复核
28. Draft Management UI
29. Human Review UI
```

> 本 Phase 详细任务与验收标准以路线图 [Phase 7：问题池、信源发现与文章生成 MVP](GEO_Agent_长期开发路线图.md) 为准。
...
#### Phase 9：Agent-first Task Runtime + Skill Package 编排

```text
30. Agent-first Task Runtime
31. AllowedActionPolicy / Action Registry
32. Dynamic Tool Registry
33. ToolGuard
34. ResultValidator
35. RetryManager / LoopGuard / RecoveryManager
36. Skill Package 注册与编排（GEO Skill、Search Skill、Draft Skill 等）
37. Agent Runtime 与已有 Assistant / 工具 / 审批 / 记忆系统对接
```
...
### 20.5 DeepAgents checkpointer 与 SQLite 分工
...
### 20.11 ToolGuard 与 Human Approval
